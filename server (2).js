/**
 * KRAKN·AI — Trading Bot Backend Server v3.0
 * =============================================
 * Phase 1 upgrades:
 * - P&L tracking (stores all trades, calculates profit/loss)
 * - Multi-indicator analysis (MACD + Bollinger Bands + Volume + RSI)
 * - Multi-timeframe consensus (15min + 1hr + 4hr must agree)
 * - Stop-loss protection (auto-sell if price drops X% from avg buy)
 */
...
let stopLossConfig = {
  enabled:       true,
  dropPct:       8,     // sell if price drops 8% from avg buy price
  trailingStop:  false, // future feature
};
...
app.post('/api/stoploss', requireAuth, (req, res) => {
  ...
  res.json({ success: true, data: { enabled: botConfig.stopLossEnabled, dropPct: botConfig.stopLossPct } });
});
    ...
    try {
      ...
      if (botConfig.stopLossEnabled && pnl.avgBuyPrice > 0) {
        ...
        if (dropPct <= -botConfig.stopLossPct) {
          ...
          await executeSell(holding, ticker, 'stop-loss', `Stop-loss triggered at ${dropPct.toFixed(1)}% drop`, dp);
          continue;
        }
      }
...
app.listen(PORT, () => {
  console.log('');
  console.log('╔════════════════════════════════════════╗');
  console.log('║        KRAKN·AI Bot Server v3.0        ║');
  console.log('╠════════════════════════════════════════╣');
