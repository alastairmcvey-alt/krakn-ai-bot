/**
 * KRAKN·AI — Trading Bot Backend Server v3.1
 * =============================================
 * Phase 1: P&L tracking, multi-indicator signals, stop-loss, multi-timeframe
 * Phase 2: Persistent settings, Fear & Greed sentiment, trailing stop-loss, DCA bot
 */
...
const DATA_FILE = path.join(__dirname, 'krakn_data.json');
...
let stopLossPeaks = {}; // { sym: highestPriceSinceBuy } for trailing stop

// ─── DCA (Dollar Cost Averaging) Bot ──────────────────────────
let dcaConfig = {
  enabled:       false,
  pairs:         ['XBTAUD'],       // which coins to DCA into
  amountAUD:     50,               // how much AUD to spend per run
  frequency:     'weekly',         // 'daily' | 'weekly' | 'monthly'
  dayOfWeek:     1,                // 0=Sun 1=Mon ... for weekly
  hour:          9,                // hour to run (AEST)
  lastRun:       null,
  totalSpent:    0,
};
let dcaTimer = null;
...
function loadData() {
  try {
    if (!fs.existsSync(DATA_FILE)) { console.log('[LOAD] No saved data — using defaults'); return; }
    ...
    console.log(`[LOAD] ✅ Settings restored — risk:${botConfig.riskLevel} confidence:${botConfig.confidenceMin}% stopLoss:${botConfig.stopLossPct}%`);
...
async function analyseTimeframe(pair, interval) {
  ...
  return { interval, rsi, macd, bb, volSig, score, signals };
}
...
async function computeSignalForPair(pair) {
  try {
    ...
    return {
      ...
      // Keep rsi for backwards compatibility
      rsi: tf60.rsi,
    };
  } catch(e) {
...
async function runAdvisor() {
  ...
  try {
    ...
    const fgContext     = fearGreed.value
      ? `\nMarket Sentiment — Fear & Greed Index: ${fearGreed.value}/100 (${fearGreed.label})${fearGreed.value <= 25 ? ' — Extreme Fear, historically good buying opportunity' : fearGreed.value >= 75 ? ' — Extreme Greed, market may be overheated' : ''}`
      : '';
...
app.post('/api/ai/signal', requireAuth, async (req, res) => {
  try {
    ...
    const fearGreed = await fetchFearGreed();
    const fgNote    = fearGreed.value
      ? `\nMarket Sentiment: Fear & Greed Index ${fearGreed.value}/100 (${fearGreed.label})`
      : '';
...
Analyse ${displayPair} at ${fmtAUDServer(parseFloat(price))} (${change24h > 0 ? '+' : ''}${change24h}% 24h, RSI: ${rsi}).${balanceNote}${fgNote}
...
app.get('/api/pnl', requireAuth, async (req, res) => {
  try {
    // Pull trade history from Kraken to seed P&L if tradeLog is empty
    if (tradeLog.length === 0 && KRAKEN_API_KEY && KRAKEN_API_SECRET) {
      try {
        const history = await krakenPrivateRequest('TradesHistory');
        const trades  = Object.values(history.trades || {}).slice(0, 100);
        for (const t of trades) {
          const sym = (t.pair||'').replace('AUD','').replace(/^X/,'').replace(/Z$/,'').replace('XBT','BTC');
          if (sym && t.price && t.vol) {
            recordTrade(t.pair, sym, t.type, t.vol, t.price, 'kraken-history');
          }
        }
    ...
    for (const [sym, data] of Object.entries(pnlByAsset)) {
      ...
      // Get current price for unrealised calc
      const pair   = AUD_PAIRS.find(p => p.includes(sym === 'BTC' ? 'XBT' : sym));
      if (pair) {
        try {
          const ticker = await fetchSingleTicker(pair);
          if (ticker && data.totalVolume > 0) {
            ...
            assetSummary.push({
              ...
              unrealisedPnl: unreal.unrealisedPnl,
              pnlPct:        unreal.pnlPct,
...
app.get('/api/stoploss', requireAuth, (req, res) => {
  res.json({ success: true, data: { ...botConfig.stopLossEnabled !== undefined ? { enabled: botConfig.stopLossEnabled, dropPct: botConfig.stopLossPct } : stopLossConfig } });
});
...
// ─── Enhanced Signal Route (multi-indicator) ───────────────────
...
let botConfig = {
  ...
  stopLossPct:        8,     // sell if drops 8% from avg buy price
};
let botState = {
  running:     false,
  lastCheck:   null,
...
function scheduleDCA() {
  ...
  dcaTimer = setInterval(checkDCASchedule, 60 * 60 * 1000);
...
app.listen(PORT, () => {
  console.log('');
  console.log('╔════════════════════════════════════════╗');
  console.log('║        KRAKN·AI Bot Server v3.1        ║');
  console.log('╠════════════════════════════════════════╣');
