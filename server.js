/**
 * KRAKN·AI — Trading Bot Backend Server v3.2
 * =============================================
 ...
 * Phase 3: Portfolio history, tax export, buy/sell bot state persistence
 * Fixes: bot auto-restart, risk level persistence, buy signals independent of advisor interval
 */
...
function scheduleBuyCheck() {
  ...
  buyCheckTimer = setInterval(checkBuyOpportunity, 60 * 60 * 1000); // every hour
...
Only include coins above ${advisorSettings.minConfidence}% confidence. If none qualify say "No strong signals right now."`;
    ...
    // Only run buy check inside advisor if advisor is set to 1h
    // (if daily, buy check runs independently via scheduleBuyCheck)
...
// Tracks daily portfolio value for performance graph
...
let botConfig = {
  ...
  stopLossPct:        8,
  trailingStop:       false,  // trailing stop tracks peak price, not avg buy
};
let botState = {
  running:     false,
  lastSignals: {},
...
app.listen(PORT, () => {
  console.log('');
  console.log('╔════════════════════════════════════════╗');
  console.log('║        KRAKN·AI Bot Server v3.2        ║');
  console.log('╠════════════════════════════════════════╣');
  ...
  setTimeout(recordPortfolioSnapshot, 30000);
  setInterval(recordPortfolioSnapshot, 6 * 60 * 60 * 1000);
